document.addEventListener('DOMContentLoaded', () => {
  ...
  const STORAGE_KEY = 'bermonths_progress_v2';
  ...
  if (state.sawCover) book.classList.add('flipped');
  btnNextPage.addEventListener('click', goToCourtPage);
  if (state.sawGifts) book.classList.add('court-open');
  ...
  /* ADD AUDIO SRC HERE (alternative to editing the <audio> tag above):
     audio.src = "balang-araw.mp3"; */
  ...
  function playSong() {
    if (!audio.src) { alert('add your audio file! see the comment above the <audio> tag 🎵'); return; }
    audio.play().catch(() => {});
    btnPlay.textContent = '⏸';
